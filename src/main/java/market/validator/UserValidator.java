package market.validator;

import market.model.Customer;
import market.model.Product;
import market.model.User;
import market.service.CustomerService;
import market.service.ProductService;
import market.service.UserService;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;
import org.springframework.validation.Errors;
import org.springframework.validation.ValidationUtils;
import org.springframework.validation.Validator;

@Component
public class UserValidator implements Validator {
    @Autowired
    private UserService userService;
    @Autowired 
     CustomerService customerservice;  
    @Autowired
    ProductService productservice;
    
    public boolean supports(Class<?> aClass) {
        return User.class.equals(aClass);
    }
    public boolean support(Class<?> aClass){
    	return Customer.class.equals(aClass);
    }
    public boolean supportss(Class<?> aClass){
    	return Product.class.equals(aClass);
    }
    public void validate(Object o, Errors errors) {
        User user = (User) o;

        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "username", "NotEmpty");
        if (user.getUsername().length() < 6 || user.getUsername().length() > 32) {
            errors.rejectValue("username", "Size.userForm.username");
        }
        if (userService.findByUsername(user.getUsername()) != null) {
            errors.rejectValue("username", "Duplicate.userForm.username");
        }

        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "password", "NotEmpty");
        if (user.getPassword().length() < 8 || user.getPassword().length() > 32) {
            errors.rejectValue("password", "Size.userForm.password");
        }

        if (!user.getPasswordConfirm().equals(user.getPassword())) {
            errors.rejectValue("passwordConfirm", "Diff.userForm.passwordConfirm");
        }
    }
    public void validateCus(Object o,Errors errors){
    	Customer customer=(Customer) o;
    	if(customer.getName()==null){
    		errors.rejectValue("name", "Notnull.neworder.name");
    	}
    	ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "NotNullName");
    	ValidationUtils.rejectIfEmptyOrWhitespace(errors, "email", "NotNullEmail");
    	ValidationUtils.rejectIfEmptyOrWhitespace(errors, "phonenumber", "NotNullPhone");
    	ValidationUtils.rejectIfEmptyOrWhitespace(errors, "streetname", "NotNullAdd");
    }
    public void validateProduct(Object o, Errors errors){
    	//Product product =(Product) o;
    	ValidationUtils.rejectIfEmpty(errors, "name", "NotEmptyNameProduct");
    	ValidationUtils.rejectIfEmpty(errors, "description", "NotEmptyDescription");
    	ValidationUtils.rejectIfEmpty(errors, "image", "NotEmptyImage");
    	ValidationUtils.rejectIfEmpty(errors, "price", "NotEmptyPrice");
    }
}
